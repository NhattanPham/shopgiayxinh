...
@section('content')
    <div class="box-container">
        <header class="page-header">
            <h1 class="title-header">Danh sách Menu</h1>
            <ul class="button-header">
                <li><a class="uk-button uk-button-success" href="{{ url('/admin/menu/create') }}">Thêm mới</a></li>
            </ul>
        </header>
                ...
                <div class="toolbar">
                    <div class="action-toolbar">
                        <select name="action">
                            ...
                            <option value="delete">Xóa menu</option>
                        </select>
                    ...
                    <div class="filter-toolbar">
                        <div class="search-toolbar">
                            ...
                            <button class="uk-button" type="button" onclick="clean('{{ url('admin/menu') }}')">Làm
                                sạch</button>
                        </div>
                        <?php $limit_option = option('limit_menu'); ?>
                        <select class="numberPage"
                            onchange="javascript:jQuery(this).changeNumbePage('/admin/config/limited/' + this.value+'/limit_menu')">
                ...
                <div class="content">
                    @if (count($menu) > 0)
                                ...
                                <tbody>
                                    @foreach ($menu as $item)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ url('admin/menu/edit/' . $item->id) }}">{{ $item->name }}</a>
                                            </td>
                                            <td>{{ $item->slug }}</td>
                        ...
                        <div class="pagination">
                            @include('backend.partials.pagination', ['paginator' => $menu])
                        </div>
                    @endif
                </div>
